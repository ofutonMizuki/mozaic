...
- 変換ターゲット: **当面 C++ 一本**(OS 不問:g++/clang++)。GPU は Apple Silicon/Metal。WASM / JS は将来構想

## できること(現状 / M0)

`mozaic → C++ → ネイティブ実行`。標準入出力のみの極小ランタイム。

- **型**(厳密・暗黙変換なし): `i8`..`u64` / `f32` `f64` / `bool` / `str`(`char`/UTF-32 文字型は SPEC 上の設計で、M0 では未実装)
...
- **カーネル(データ並列)**: `kernel`、`Buffer<T>`、`launch(k, grid, ...)`。**CPU と Apple Silicon GPU(Metal)の両対応**。グリッドは 1D(整数)/ 2D `grid2(w,h)` / 3D `grid3(w,h,d)`、カーネル内は `grid.{x,y,z}`
...
[examples/](examples/): echo / sum / fib / funcs / floats / struct / enums / overflow / addk(カーネル)/ **addk_async**(`dev.launch`+`await`、CPU/GPU 両対応)/ **matadd**(2D グリッド `grid2`)/ **borrow_err**(借用=同期のコンパイルエラー例)。
