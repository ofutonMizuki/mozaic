...
対応構文: `fn main`、`for (const x of stdin.lines())`、`if` / `==` / `!=`、`stdout.println(...)`、`return` / `break` / `continue`。
...
```sh
node src/main.ts emit  examples/echo.moz   # 生成された C++ を表示
node src/main.ts build examples/echo.moz   # build/echo を生成
node src/main.ts run   examples/echo.moz   # ビルドして実行
```
