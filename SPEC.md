...
- **実装状況(v0.1 / Phase 1)**:再束縛(`let q = p` / `p = q`)での**ムーブと use-after-move 検査**が入っている。`Copy` = スカラ/`bool`、それ以外(`str`/`Buffer<T>`/`struct`/ペイロード付き `enum`/`Atomic`)はムーブ。関数引数の値渡しは当面コピー(参照 `&T`/`&mut T` の導入でムーブへ移行予定)。`Atomic` はムーブ不可。一級参照・別名規則・escape 検査は後続フェーズ(§8 TBD → 順次確定)。
