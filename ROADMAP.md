...
| **M7** | セルフホスト ★完成(実用サブセット ✅ / 全機能版残) | ✅ **mozaic で書いた mozaic→C++ コンパイラ** [selfhost.mzc](tests/cases/selfhost.mzc)(lex→再帰下降 parse(`Box` 再帰 AST)→C++ emit)。対応: 関数+再帰 / int・bool・str / **struct** / **enum + match** / **固定配列 [T;N]** / 制御フロー / 全演算子 / 呼出し / println。再帰・struct・文字列・enum/match・配列プログラムを **clang++ で通る C++ に変換し正しく実行**(実機検証)。残: generics/atomic/comptime/optional/Result/Vec・Box(被コンパイル側)/GPU 等へ拡張し、生成 C++ で既存ゴールデン全件を通す(=型検査器込みの全面移植) | M2 + M3 + M6 | 最大 |
