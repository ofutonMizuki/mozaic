...
- ゴールデンテスト **40/40 pass**(`.out` 23 / `.err` 17, うち 5 は `.emit` 表明併用)。
...
| M | テーマ | 主な中身 | 依存 | 規模 |
|---|---|---|---|---|
| **M2** | 言語コア充実 | `as`/`as?` キャスト ・ `T?`(`some`/`none`/`??`/後置 `?`)・ `Result<T,E>`(`Ok`/`Err`/`?`/コンビネータ)・ `defer` ・ 固定長配列 `[T;N]` / スライス `[]T` ・ **`char` 型 + 文字リテラル**(未実装)・ `str`/`String` 分離 + メソッド + テンプレート文字列 `` `…${e}…` `` ・ `abort`/`assert` | 現在地 | 大 |
| **M3** | 抽象化 | `comptime` ・ ユーザ総称型(generic fn/struct)・ ベクタ/SIMD(`f32x4` + `splat`、CPU SIMD / GPU レーン)・ `i128`/`u128`/`f16` | M2 | 大 |
