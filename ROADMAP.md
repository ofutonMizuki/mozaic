...
- **M7**: mozaic 製コンパイラ(`tests/cases/selfhost.mzc`, "mozc")の自己ホスト。
  ...
  - 残: 負例ゴールデン 41 個の **拒否**。mozc は 13/41 を間接的に拒否(parse error / 生成 C++ が
    コンパイル不能)、28 個を取りこぼす(move/borrow/alias/escape/Sync/型規則)。これらを捕まえるには
    型/借用/Sync チェッカ(`check.ts` 約950行)を mozaic へ移植する必要があり、mozc を再設計する
    大規模・複数セッションの作業(mozc は設計上チェッカを持たず C++ テンプレート推論に委ねている)。
