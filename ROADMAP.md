...
- **M7 完了**: mozaic 製コンパイラ(`tests/cases/selfhost.mzc`, "mozc")の自己ホスト。**完成**。
  ...
  - ★★ **正例ゴールデン 61/61 を mozc が正しくコンパイル**(`./mozc-coverage.sh`)= 言語の正例全面
  ...
  - ★★★ **負例ゴールデン 41/41 を mozc が拒否**(`./mozc-neg-coverage.sh`)。mozc に
  ...
  - ⇒ **mozc は 102/102 ゴールデン全てをカバー**(正例コンパイル + 負例拒否)。実コンパイラ
    (`src/*.ts`)も 102/102 のまま。言語は **完成**。
