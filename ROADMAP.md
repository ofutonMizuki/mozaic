...
> 正例には 3D グリッド `grid3_basic`・真の 128bit `wide128`・直接 128bit リテラル `wide128_lit`・検査付き算術 `checked_arith` を追加。
>
> **2026-06-03 健全性ハードニング**(差分テストで発見・修正した実装漏れ 2 件):
> - **>2^64 整数リテラル**: 参照コンパイラは `Num` リテラルを `renderInt` 経由で `((hi<<64)|lo)` 合成するよう修正
>   (従来は生のリテラルを吐き clang が "integer literal too large" で拒否)。`wide128_lit` で回帰防止。
> - **mozc の検査付き算術**: mozc は `+ - * / %` を型推論型の `mz::chk_*`(`decltype(a OP b)` で両オペランドの
>   昇格型を取る)へ降ろすよう修正。これで mozc 生成コードも **debug で overflow trap・release で wrap**(参照と一致)、
>   かつ符号付きオーバーフロー UB を解消。release では `__builtin_*_overflow` の判定が消えネイティブ `add` と同一命令。
>   *既知の subset 限界*: mozc は型情報を持たないため直接 >2^64 リテラルは非対応(算術で構築する `wide128` の流儀を使う; `wide128_lit` は参照専用)。
...
- **M7 全機能版 ★(最大の残作業)**: 自己ホスト不動点はサブセットで達成済み。残るは selfhost.mzc を
  ...
  相当を mozaic で書き切って生成 C++ で既存 105 ゴールデン**全件**を通すこと(=型検査器込みの全面移植、数千行規模)。
...
- **M7 完了**: mozaic 製コンパイラ(`tests/cases/selfhost.mzc`, "mozc")の自己ホスト。**完成**。
  - ★ 自己再生成 FIXPOINT 到達(gen2==gen3, `./selfhost-check.sh`)。
  - ★★ **正例ゴールデン 63/63 を mozc が正しくコンパイル**(`./mozc-coverage.sh`)= 言語の正例全面
    (generics, Atomic/Mutex/Channel, spawn/scope, Task, optional `?`/`as?`, Result, comptime,
    SIMD, GPU kernel+launch+workgroup, 複数ファイル import, i128/u128, for-of, u32 文字列)。
  - ★★★ **負例ゴールデン 42/42 を mozc が拒否**(`./mozc-neg-coverage.sh`)。mozc に
    ゼロから書いた型/借用/Sync チェッカを実装:move(use-after-move / loop-move)、
    別名(同一 call 内の `&mut`×2 / `&`+`&mut`)、const 変更不可、参照/スライスの局所エスケープ、
    Send/Sync(spawn 越境)、Task の join 漏れ、Atomic 制約、comptime のランタイム値読みを検出。
  - ⇒ **mozc は 105/105 ゴールデン全てをカバー**(63 正例コンパイル + 42 負例拒否)。実コンパイラ
    (`src/*.ts`)も 105/105 のまま。言語は **完成**。
